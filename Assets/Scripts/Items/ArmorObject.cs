    ...
    public class ArmorObject : ItemObject
    {
        ...
        private void OnTriggerStay2D(Collider2D collision)
        {
            if (collision.tag == "Player")
            {
                ...
                {
                    // determine which slot group to check based on Item.Armor.Type


                    // determine if inv slot group has any unoccupied slots
                    // Inventory.Instance.GetNextAvailableSlot()
                    ...
                    //{
                    //    ItemCreator.Instance.CreateArmor(quality, type);
